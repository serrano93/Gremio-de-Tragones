...
interface MissionQRGeneratorProps {
...
export function MissionQRGenerator({ mission, userId, isOpen, onClose }: MissionQRGeneratorProps) {
  ...
  if (!mission || !payload) return null

  const qrString = JSON.stringify(payload)
  const estName = mission.establishment?.name || 'Establecimiento'

  return (
    <Modal isOpen={isOpen} onClose={onClose} title="Sello de Misión" size="md">
      <div className="flex flex-col items-center gap-md">
        <p className="font-body-md text-on-surface-variant text-center max-w-md mx-auto mb-md">
          Presenta este glifo ante el Tabernero para reclamar tu recompensa.
        </p>
        ...
        <div className="relative p-md bg-surface-container-high border-2 border-outline-variant">
          ...
          >
            <QRCodeCanvas
              ...
              level="M"
              includeMargin
              aria-label={`Código QR para la misión: ${mission.title}`}
            />
          </motion.div>
        </div>
        ...
        <div className="parchment-texture w-full max-w-md p-md rounded-lg border-2 border-[#2a2a2a] relative overflow-hidden">
          ...
          <div className="flex justify-between items-start mb-sm">
            <div>
              ...
              <span className="font-label-sm text-label-sm uppercase tracking-widest text-on-tertiary-fixed-variant">Misión de Gremio</span>
            </div>
          ...
          <p className="font-body-md text-on-tertiary mb-md leading-relaxed">{mission.description || mission.title}</p>
          <div className="flex items-center gap-sm pt-sm border-t border-black/20">
            <span className="material-symbols-outlined text-on-tertiary">military_tech</span>
            <span className="font-label-lg text-label-lg text-on-tertiary">RECOMPENSA: {mission.xp_reward} XP</span>
          </div>
        </div>
