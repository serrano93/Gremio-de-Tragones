...
import { AUTH_STORAGE_KEY, supabaseUrlValue as supabaseUrl, supabaseAnonKeyValue as supabaseAnonKey } from '../lib/supabase'
        ...
        if (sessionStr) {
          try {
            ...
            const res = await fetch(
              `${supabaseUrl}/rest/v1/profiles?auth_id=eq.${session.user.id}`,
              {
                method: 'PATCH',
                headers: {
